...
在 [[Developer_Agentic_Workflow|开发者 Agentic AI 工作流]] 中，核心最佳实践是 **plan-first 模式**：

1. 开新 feature → 把任务丢给 AI，**先让它写 plan**（不直接写 code）
2. Review plan、修正方向 → 才 approve 写 code
3. 写完跑 tests + lint → 自己 review diff（不要 blind accept）

> 跨多个 commit 才发现方向错是常见 anti-pattern。**plan-first** 模式可有效避免。
...
| 用 AI 生 test 后不检查 assertion | 跑覆盖率 + 故意改 bug 验证 |

## 常用工具生态

Agentic Coding 的工具生态按 [[Developer_Agentic_Workflow]] 的 Tier 路径分类：

- **Tier 0**（IDE 辅助）：[[Cursor]] — 编辑器内 AI 结对编程
- **Tier 1**（CLI agent）：[[Claude_Code]] — 终端 AI 编码 Agent；[[Aider]] — git-native CLI pair-programmer；[[Cline]] — VS Code autonomous agent
- **Tier 1-2**（替代方案）：[[OpenHands]] — 全自主 agent；[[Goose_AI]] — 多接口自主 agent；[[Roo_Code]] — VS Code multi-mode agent
- **Tier 2-3**（团队/CI）：[[Continue_Dev]] — CI source-controlled checks；[[Repomix]] — 打包 codebase 给 reviewer

## 关联连接

- [[Developer_Agentic_Workflow]] — 开发者场景分类与工具链框架
...
- [[AI_Mastery_Compass]] — AI 大模型驾驭进阶罗盘，面向 Agentic Coding 的目标拆解与迭代循环
- [[Progressive_Disclosure]] — 渐进式披露架构，解决 Agentic Coding 中提示词膨胀问题的关键模式
- [[ADK]] — Google Agent Development Kit，支持 Agentic Coding 范式的主流框架
- [[摘要-ai-era-git-management]] — 来源文章
- [[摘要-agentic-ai-roadmap]] — Agentic AI 从零到入门的系统学习路线
- [[摘要-awesome-agentic-ai-zh-for-developer]] — 开发者延伸路线来源
- [[Cursor]] — 编辑器集成 AI 结对编程（Tier 0）
- [[Aider]] — git-native CLI pair-programmer（Tier 1）
- [[Cline]] — VS Code autonomous agent（Tier 1）
- [[OpenHands]] — 开源自主 agent（Tier 1-2）
- [[Goose_AI]] — 开源可扩展 AI agent（Tier 1-2）
- [[Roo_Code]] — VS Code multi-mode agent（Tier 1-2）
- [[Continue_Dev]] — CI source-controlled checks（Tier 2-3）
- [[Repomix]] — codebase 打包工具
