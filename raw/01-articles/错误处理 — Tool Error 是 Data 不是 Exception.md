...
| 谁决定重试 | Python 层（try/except） | **LLM 层**（看 category 后自主决策） |
| recover 机会 | 无，直接 crash | 有，LLM 可重试/改 query/放弃 |
| 信息量 | "failed"（看不懂） | `category` + `retry_hint`（可理解） |
| 小模型友好 | ❌ | ✅ |

> **Production 的 retry 不在 Python 层、而在 LLM 层——这个 mental flip 是练习 5 的核心。**
...
def fetch_weather(args: dict) -> dict:
    ...
    error 也是 data，LLM 需要看到结构化错误信息才能做决策。
    """
...
for step in range(8):
    ...
    for tc in msg.tool_calls:
        ...
        if retry_counts[tool_name] > RETRY_LIMIT:
            ...
            print(f"  [BLOCKED] 超过 retry 上限 ({RETRY_LIMIT}次)")
        else:
...
Step 2
============================================================
[Thought] 查询台北天气遇到了网络连接问题，请稍后再试。
提示您可以等待大约1到2秒尝试重新获取信息。

[Final Answer] 查询台北天气遇到了网络连接问题，请稍后再试。
```

（由于 `random.random()`，每次运行结果不同——可能是 fatal 错误、可能第一次就成功、也可能出现 3 次 transient 后被强制拦停。）

---

## 三种错误场景解析

### 场景 A：Transient → LLM 重试 → 成功

```
Step 1: fetch_weather("台北")
        → {"error": "网络超时", "category": "transient", ...}

Step 2: fetch_weather("台北")    ← LLM 看到 category 后决定重试
        → {"city": "台北", "forecast": "rain", "temperature_c": 24, "status": "ok"}

Step 3: 无 tool_calls
        → Final Answer: "台北今天下雨，24°C"
```

### 场景 B：Transient × 3 → Python 层强制终止

```
Step 1: transient    ← LLM 决定重试
Step 2: transient    ← LLM 决定重试
Step 3: transient    ← LLM 决定重试
Step 4: [BLOCKED] 超过 retry 上限 (3次)
        → {"error": "工具 fetch_weather 已重试 3 次仍失败",
           "category": "fatal",
           "retry_hint": "请放弃使用此工具"}

Step 5: 无 tool_calls
        → Final Answer: "天气服务不稳定，重试 3 次后仍失败"
```

### 场景 C：Fatal → LLM 放弃 → 直接回答

```
Step 1: fetch_weather("台北")
        → {"error": "城市 '台北' 不在服务覆盖范围内",
           "category": "fatal",
           "retry_hint": "请检查城市名称是否正确"}

Step 2: 无 tool_calls
        → Final Answer: "台北不在当前天气服务的覆盖范围内，无法查询"
```
...
def fetch_weather(args):
    if network_failed():
        raise ConnectionError("timeout")  # ← 循环崩溃

try:
    obs = fetch_weather(args)
except Exception:
    obs = "failed"                       # ← LLM 看不懂 "failed"
```
...
def fetch_weather(args):
    if network_failed():
        return {"error": "timeout", "category": "transient"}

obs = fetch_weather(args)                # ← 永远不 crash
# ↑ 传给 LLM，LLM 看到结构化 dict，自己决定下一步
```
...
结构化 dict 的字段名本身就是提示词——LLM 看字段名就知道含义。
...
```python
# 1. raise Exception          → 循环崩溃
raise ConnectionError("timeout")

# 2. return "failed"          → LLM 看不懂
return "failed"

# 3. return "网络超时，请重试"  → LLM 能看懂但无法结构化解析
return "网络超时，请重试"

# 4. return {...}             → LLM 可理解每个字段
return {"error": "网络超时", "category": "transient", "retry_hint": "稍后重试"}
```
...
从实际运行中可以看出 qwen2.5:3b 的典型反应：

| 场景 | qwen2.5:3b 的行为 | GPT-4/Claude 的典型行为 |
|------|------------------|----------------------|
| 看到 transient | 倾向直接告诉用户"稍后重试" | 主动再次调用工具 |
| 看到 fatal | 正确告诉用户原因 | 正确告诉用户原因 |
| 看到 retry_hint | 有时照做，有时忽略 | 通常按 hint 执行 |
| 连续失败后 | 容易放弃而非坚持重试 | 可能重试多次再放弃 |

这不是代码问题——小模型在"推断工具调用意图"上的能力天然弱于大模型。但如果连小模型都能看到结构化错误并做出合理响应，说明设计是好的。
